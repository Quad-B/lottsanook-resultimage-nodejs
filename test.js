            ...
            request(options, async function (error, response) {
                ...
                if (url.parse(req.url, true).query.bgimg) {
                    ...
                    if (thisistoday) {
                        ...
                        await new Pageres({ delay: 1, filename: datecheck.substring(0, 2) + '-' + datecheck.substring(2, 4) + '-' + datecheck.substring(4, 8)+ '_' + bgurl, launchOptions: { args: ['--no-sandbox', '--disable-setuid-sandbox', '--no-first-run', '--disable-extensions'] } })
