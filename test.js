        ...
        (async () => {
            ...
            request(options, async function (error, response) {
                ...
                if (url.parse(req.url, true).query.bgimg) {
                    ...
                    res.writeHead(200, { 'content-type': 'image/png' });
                    fs.createReadStream(datecheck.substring(0, 2) + '-' + datecheck.substring(2, 4) + '-' + datecheck.substring(4, 8) + '_' + padLeadingZeros(Math.floor(Math.random() * 999), 3) + '_cbg.png').pipe(res);
