        ...
        (async () => {
            ...
            request(options, async function (error, response) {
                ...
                if (url.parse(req.url, true).query.bgimg) {
                    ...
                    if (thisistoday) {
                        let rnum = padLeadingZeros(Math.floor(Math.random() * 99), 2)
                        ...
                        await new Pageres({ delay: 1, filename: datecheck.substring(0, 2) + '-' + datecheck.substring(2, 4) + '-' + datecheck.substring(4, 8) + '_' + rnum, launchOptions: { args: ['--no-sandbox', '--disable-setuid-sandbox', '--no-first-run', '--disable-extensions'] } })
                        ...
                        res.writeHead(200, { 'content-type': 'image/png' });
                        fs.createReadStream(datecheck.substring(0, 2) + '-' + datecheck.substring(2, 4) + '-' + datecheck.substring(4, 8) + '_' + rnum + '.png').pipe(res);
                        console.log('Finished generating screenshots!');
                    } else {
                        res.writeHead(200, { 'content-type': 'image/png' });
                        fs.createReadStream(datecheck.substring(0, 2) + '-' + datecheck.substring(2, 4) + '-' + datecheck.substring(4, 8) + '_' + rnum + '.png').pipe(res);
