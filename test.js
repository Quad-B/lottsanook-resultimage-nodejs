        ...
        (async () => {
            ...
            request(options, async function (error, response) {
                ...
                if (url.parse(req.url, true).query.bgimg) {
                    await new Pageres({ delay: 2, filename: datecheck.substring(0, 2) + '-' + datecheck.substring(2, 4) + '-' + datecheck.substring(4, 8), launchOptions: { executablePath: '/usr/bin/chromium-browser', args: ['--no-sandbox', '--disable-setuid-sandbox', '--no-first-run', '--disable-extensions'] } })
